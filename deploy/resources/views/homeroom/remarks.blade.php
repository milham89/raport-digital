...
<div class="space-y-6">
    <div class="card flex justify-between items-center">
        <div>
            <h2 class="font-bold text-slate-800">Catatan & Absensi Kelas {{ $class->name ?? '' }}</h2>
            <p class="text-xs text-slate-500">Tahun Ajaran: {{ $activeYear?->year }} ({{ $activeYear?->semester }})</p>
        </div>
        <a href="{{ route('homeroom.dashboard') }}" class="btn-secondary">&larr; Kembali</a>
    </div>
            ...
            <div class="overflow-x-auto"><table class="w-full min-w-[700px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="table-cell py-3"><input type="text" name="remarks[{{ $s->id }}][homeroom_note]" value="{{ $rem?->homeroom_note ?? '' }}" placeholder="Masukkan catatan perkembangan..." class="input !py-1.5 text-sm"></td>
                    </tr>
                    @endforeach
                </tbody>
            </table></div>
