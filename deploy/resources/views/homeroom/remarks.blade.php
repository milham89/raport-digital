...
<div class="space-y-6">
    <div class="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="font-bold text-slate-800 dark:text-slate-100 text-lg">Catatan & Absensi Kelas {{ $class ? $class->name : '' }}</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Tahun Ajaran: {{ $activeYear ? $activeYear->year : '' }} ({{ $activeYear ? $activeYear->semester : '' }}) &bull; Nilai S/I/A akan tertera di cetak raport.</p>
        </div>
    ...
    <div class="card overflow-hidden p-0">
        <form method="POST" action="{{ route('homeroom.remarks.store') }}">
            ...
            <div class="overflow-x-auto"><table class="w-full min-w-[700px]">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                        <td class="table-cell font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap py-3">
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
                <p class="text-xs text-slate-500 dark:text-slate-400">Nilai Sakit, Izin, dan Alpa disinkronkan otomatis dari Absensi Harian atau dapat disesuaikan manual.</p>
