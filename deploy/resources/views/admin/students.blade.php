...
<div class="space-y-6" x-data="{ modalOpen: false, modalStudent: null, modalName: '' }">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="card h-fit">
            <h3 class="font-bold text-slate-800 mb-4 text-base">+ Tambah Siswa & Akun</h3>
            <form method="POST" action="{{ route('admin.students.store') }}" class="space-y-3.5">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 mt-1">Siswa login menggunakan NIS / Email dan password ini.</p>
                </div>
        ...
        <div class="lg:col-span-2 card overflow-hidden p-0">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h3 class="font-bold text-slate-800">Daftar Siswa Terdaftar</h3>
                <span class="badge bg-blue-50 text-blue-700">{{ $students->count() }} Siswa</span>
            </div>
            <div class="overflow-x-auto"><table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="table-cell font-mono text-xs text-slate-600"><b>{{ $s->nis }}</b><br><span class="text-slate-400">{{ $s->nisn }}</span></td>
                        <td class="table-cell font-semibold text-slate-800">{{ $s->name }} ({{ $s->gender }})</td>
                        <td class="table-cell"><span class="badge bg-slate-100 text-slate-700">{{ $s->schoolClass->name ?? '-' }}</span></td>
                        <td class="table-cell text-xs">
                            @if($s->userAccount)
                                <span class="badge bg-emerald-50 text-emerald-700">Aktif</span>
                                <span class="block text-[11px] text-slate-400 mt-0.5">{{ $s->userAccount->email }}</span>
                            @else
                                <span class="badge bg-rose-50 text-rose-700">Belum Ada Akun</span>
                            @endif
                        </td>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" style="display:none">
        <div class="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl" @click.outside="modalOpen=false">
            <h3 class="text-base font-bold text-slate-800 mb-1">Set Password Akun Siswa</h3>
            <p class="text-xs text-slate-500 mb-4" x-text="'Siswa: ' + modalName"></p>
