...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="card h-fit">
        <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 text-base">+ Tambah Tahun Ajaran</h3>
        <form method="POST" action="{{ route('admin.academic-years.store') }}" class="space-y-4">
            ...
            <div class="flex items-center gap-2 pt-1">
                <input type="checkbox" name="is_active" value="1" id="is_act" class="rounded border-slate-300 dark:border-slate-700 text-blue-600">
                <label for="is_act" class="text-sm font-medium text-slate-700 dark:text-slate-300">Set Sebagai Aktif</label>
            </div>
        ...
        <div class="overflow-x-auto"><table class="w-full">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                    <td class="table-cell font-semibold text-slate-800 dark:text-slate-200">{{ $year->year }}</td>
                    <td class="table-cell font-medium">{{ $year->semester }}</td>
                    <td class="table-cell text-center"><span class="badge {{ $year->is_active ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' }}">{{ $year->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
