...
<div class="space-y-6">
    <div class="card flex justify-between items-center">
        <div>
            <h2 class="font-bold text-slate-800 text-lg">Rekap Capaian Kelas {{ $class->name }}</h2>
            <p class="text-xs text-slate-500">Wali Kelas: {{ $class->homeroomTeacher->name ?? '-' }} &bull; Tahun: {{ $activeYear?->year }} ({{ $activeYear?->semester }})</p>
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="table-cell text-center">
                        <span class="badge {{ is_numeric($avg) && $avg >= 75 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">
