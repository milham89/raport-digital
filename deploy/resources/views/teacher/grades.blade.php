...
<div class="space-y-6">
    <div class="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-lg font-bold text-slate-800">{{ $assignment->subject->name }} - Kelas {{ $assignment->schoolClass->name }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">Tahun Ajaran: {{ $assignment->academicYear->year }} ({{ $assignment->academicYear->semester }}) &bull; KKM: <span class="font-bold text-blue-600">{{ $assignment->subject->kkm }}</span></p>
        </div>
            ...
            <div class="overflow-x-auto"><table class="w-full min-w-[840px]">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        <td class="table-cell font-mono text-xs text-slate-500 whitespace-nowrap">{{ $student->nis }}</td>
                        <td class="table-cell font-semibold text-slate-800 whitespace-nowrap">{{ $student->name }}</td>
                        ...
                        <td class="table-cell text-center font-bold text-base text-blue-600">{{ $grade?->final_score ?? '-' }}</td>
                        <td class="table-cell text-center">
                            @if($grade?->letter_grade)
                                <span class="badge {{ in_array($grade->letter_grade, ['A','B']) ? 'bg-emerald-50 text-emerald-700' : ($grade->letter_grade === 'C' ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700') }}">{{ $grade->letter_grade }}</span>
                            @else
